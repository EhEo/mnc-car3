...
1. **대시보드**
   - 직원 및 차량 상태 통계 (근무중/퇴근완료, 운행대기/운행중)
   - 오늘의 탑승 기록 조회
   - 시스템 초기화 기능
...
5. **실시간 통계**
   - 직원 상태별 카운트 (근무중/퇴근완료)
   - 차량 상태별 카운트 (운행대기/운행중/퇴근완료)
   - 오늘의 탑승 건수

### 🔄 미구현 기능

- 자동 초기화 (현재는 수동 초기화만 지원)
- 실시간 푸시 알림
- 과거 기록 조회 및 통계
- 사용자 인증/권한 관리
...
- `name`: 직원 이름
- `department`: 부서명
- `status`: 상태 (working: 근무중, left: 퇴근완료)
- `created_at`: 등록일시
- `updated_at`: 수정일시
...
- `driver_name`: 기사명
- `driver_phone`: 전화번호
- `status`: 상태 (waiting: 운행대기, driving: 운행중, completed: 퇴근완료)
- `created_at`: 등록일시
- `updated_at`: 수정일시

#### boarding_records (탑승 기록 테이블)
- `id`: 고유 ID (자동 증가)
- `vehicle_id`: 차량 ID (외래키)
...
### 탑승 관리
- `POST /api/boarding/register` - 퇴근 등록 (차량 + 여러 직원)
- `GET /api/boarding/records` - 오늘의 탑승 기록 조회
- `GET /api/boarding/stats` - 통계 정보 조회
- `POST /api/boarding/reset` - 시스템 초기화
